var RENDERER = {
	...
	setup : function(){
		...
		this.maxCount = Math.ceil(this.width * this.height * this.MAX_COUNT);
	...
	render : function(){
		...
		if(this.dynamicParticles.length < this.maxCount){
			if(--this.interval == 0){
				this.interval = this.getRandomValue(30, 30);
				this.dynamicParticles.push(new DYNAMIC_PARTICLE(this));
			}
		}
	}
...
STATIC_PARTICLE.prototype = {
	...
	render : function(context){
		...
		var gradient = context.createRadialGradient(255, 255, 255, 1,0, this.renderer.STATIC_RADIUS);
