<template>
    <div class="songsheet_items_allbox newsong_allbox">
        <span class="songsheet_container_title">歌曲列表</span>
        <div class="search_result_songs">
            <div class="search_result_song_item songer_box" v-for="(item,i) in songList" :key='i' :data-id="item.id" @click="handlePlaySong(item.id)">
                <div class="songer_index_box">
                    <span class="songer_index">{{i + 1}}</span>
                    <img :src="item.al.picUrl" alt="" class="">
                </div>
